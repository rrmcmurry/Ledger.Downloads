# Ledger Privacy Policy

Effective date: July 15, 2026

This Privacy Policy explains how the provider of The Ledger software ("Provider," "we," "us," or "our") collects, uses, shares, and protects information related to Ledger desktop accounting software, licensing, updates, downloads, support, websites, and related services.

This draft is intended for attorney review before commercial use. Replace Provider identity, address, contact information, governing law, and other business details before publication.

## 1. Our Privacy Posture

Ledger is designed as local-first desktop software. Your ledger files are intended to stay on your computer, network, removable media, or storage locations you choose.

We do not operate Ledger as a cloud accounting system. We do not provide cloud ledger storage, cloud ledger backup, cloud ledger sync, hosted ledger processing, or routine server-side access to your ledger files.

Our licensing and update services are designed to avoid collecting your accounting records. In ordinary use, we do not need and do not want your ledger files, bank registers, payroll details, tax records, client records, employee records, customer lists, vendor lists, transactions, attachments, or reports.

## 2. Information We May Collect

We may collect the following categories of information.

### Account, Purchase, and License Information

We may collect customer name, organization name, email address, license key, license type, product, product version, purchase date, payment status, activation count, activation limit, notes entered by administrators, and related license-management information.

If payments are processed through a third-party payment processor, that processor may collect payment card, bank, billing, tax, and fraud-prevention information according to its own terms and privacy policy. We should not receive full payment card numbers unless we later choose a payment setup that requires it.

### Activation Information

When you activate Ledger, the licensing service may receive information needed to validate the license and issue a machine-bound certificate. This may include license key, product, product version, activation request data, public machine key, machine fingerprint or device identifier derived by Ledger, activation date, IP address, user-agent or request metadata, and status of the activation request.

Activation data is used to issue licenses, enforce activation limits, prevent abuse, support customers, and maintain licensing records.

### Update and Download Information

When Ledger or a user checks for updates or downloads installers, we or our hosting providers may receive ordinary web request information such as IP address, date and time, requested URL, user-agent, download status, and related logs.

### Support and Feedback Information

If you contact us, submit feedback, request support, report bugs, send screenshots, send logs, or provide sample files, we may collect the information you choose to provide.

Do not send ledger files, client data, payroll data, bank data, tax data, personally identifiable information, or other confidential information unless we specifically request it and you decide it is appropriate to share. You are responsible for removing or masking sensitive information before sending support materials.

### Website Information

When we operate a website, we may collect ordinary website logs and information submitted through forms, email links, newsletter signups, checkout, trial requests, support requests, or other website features.

We may later use analytics, spam prevention, security tools, email tools, payment tools, or other service providers. If we do, this policy should be updated to identify the relevant categories of providers and data.

## 3. Information We Do Not Intentionally Collect in Ordinary Use

In ordinary licensing and update workflows, we do not intentionally collect:

- ledger database files;
- accounting transactions;
- bank statement data;
- payroll records;
- tax filings;
- customer, vendor, employee, donor, member, or client lists stored inside ledger files;
- attachments or source documents stored with or near ledger files;
- reports generated from ledger files; or
- backups of ledger files.

If you voluntarily send any of that information to us for support, testing, troubleshooting, conversion, feedback, or another purpose, we may receive and handle it for that purpose.

## 4. How We Use Information

We may use collected information to:

- provide, activate, license, and update Ledger;
- issue, verify, manage, suspend, or revoke license keys and activation certificates;
- enforce activation limits and prevent abuse;
- provide customer support;
- respond to messages, feedback, and requests;
- process purchases, refunds, chargebacks, invoices, and accounting records;
- maintain security, detect fraud, and troubleshoot systems;
- understand downloads, version adoption, update failures, and service health;
- comply with legal, tax, accounting, and recordkeeping obligations;
- enforce agreements and protect rights; and
- improve Ledger and related services.

## 5. How We Share Information

We may share information with:

- service providers that host websites, licensing services, databases, downloads, updates, email, payments, logs, security tools, analytics, or customer-support tools;
- professional advisors such as lawyers, accountants, auditors, insurers, and consultants;
- payment processors, banks, and tax or compliance providers as needed for transactions;
- law enforcement, regulators, courts, or other parties when required by law or legal process;
- parties involved in a merger, acquisition, financing, reorganization, asset sale, or transfer of the Ledger business; and
- others with your consent or at your direction.

We do not sell ledger files. We do not sell customer accounting data from ledger files because we do not intend to collect that data in the first place.

## 6. Data Security

We use reasonable administrative, technical, and organizational safeguards appropriate to the information we collect. No system is perfectly secure. We cannot guarantee that unauthorized access, disclosure, loss, misuse, or alteration will never occur.

You are responsible for securing your own computers, ledger files, backups, networks, passwords, email accounts, storage devices, and user permissions.

## 7. Data Retention

We retain information as long as reasonably necessary for the purposes described in this policy, including licensing, support, security, tax, accounting, legal, and business records.

License and activation records may be retained for the life of the license and for a reasonable period afterward so we can support activations, audits, abuse prevention, and customer questions.

Support materials you send may be retained while needed to resolve the issue and for a reasonable period afterward, unless we agree otherwise or applicable law requires a different period.

## 8. Your Choices

You may choose not to activate Ledger online. Ledger may provide offline activation workflows, but offline activation may require you to transfer activation request and response files manually.

You may choose not to send support files or feedback. If you do send materials, you may request deletion, but we may retain copies where required for legal, security, accounting, audit, dispute, or backup purposes.

You may contact us to request access, correction, or deletion of certain personal information associated with licensing, purchases, or support. We may need to verify your identity and may deny or limit requests where allowed or required by law.

## 9. Children's Privacy

Ledger is intended for business, professional, organizational, and adult personal accounting use. It is not directed to children. We do not knowingly collect personal information from children under 13. If we learn that we have collected such information, we will take reasonable steps to delete it.

## 10. International Users

Ledger and related services may be operated from the United States. If you use Ledger or related services from outside the United States, you understand that information may be processed in the United States or other countries where our service providers operate.

Attorney note: if Ledger is marketed internationally, add jurisdiction-specific terms for GDPR, UK GDPR, Canadian privacy law, Australian privacy law, and other applicable regimes.

## 11. Changes to this Policy

We may update this Privacy Policy from time to time. The updated version will be effective when posted or otherwise made available, unless a later effective date is stated. If a change materially affects how we handle personal information, we may provide notice through the Software, website, email, release notes, activation process, or other reasonable means.

## 12. Contact

Provider contact:

[Provider legal name]

[Mailing address]

[Email address]

[Website]

